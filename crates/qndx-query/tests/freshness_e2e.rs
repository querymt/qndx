...
fn freshness_e2e_modified_file() {
    ...
    let baseline_result = index_search_with_overlay(&reader, &empty_overlay, repo_path, "hello world").unwrap();
    ...
    assert_eq!(overlay.file_count(), 1, "should have 1 modified file in overlay");

    // Search for NEW content should find it (read-your-writes)
    let new_result = index_search_with_overlay(&reader, &overlay, repo_path, "goodbye world").unwrap();
    assert!(!new_result.results.matches.is_empty(), "should find new content in modified file");
    assert_eq!(new_result.stats.overlay_files, 1);

    // Search for OLD content should NOT find it in the modified file
    let old_result = index_search_with_overlay(&reader, &overlay, repo_path, "hello world").unwrap();
    assert!(old_result.results.matches.is_empty(), "should not find old content in modified file");
}
...
fn freshness_e2e_added_file() {
    ...
    fs::write(repo_path.join("new_feature.rs"), "pub const NEW_FEATURE: bool = true;\n").unwrap();

    // Build overlay
    let dirty_files = repo.detect_dirty_files().unwrap();
    let overlay = OverlayIndex::from_dirty_files(repo_path, &dirty_files, 1_000_000_000).unwrap();
    assert!(overlay.file_count() > 0, "should have added file in overlay");
    ...
    assert!(!result.results.matches.is_empty(), "should find content in new file");
    
    let paths: Vec<&str> = result.results.matches.iter().map(|m| m.path.as_str()).collect();
    assert!(paths.iter().any(|p| p.contains("new_feature.rs")), "should find in new_feature.rs");
}
...
fn freshness_e2e_deleted_file() {
    ...
    let empty_overlay = OverlayIndex::new(1_000_000_000);
    let before_result = index_search_with_overlay(&reader, &empty_overlay, repo_path, "VERSION").unwrap();
    assert!(!before_result.results.matches.is_empty(), "should find VERSION in lib.rs");
    ...
    let dirty_files = repo.detect_dirty_files().unwrap();
    let overlay = OverlayIndex::from_dirty_files(repo_path, &dirty_files, 1_000_000_000).unwrap();
    assert!(overlay.deleted_count() > 0, "should have deleted file in overlay");
    ...
    assert!(after_result.results.matches.is_empty(), "should not find content in deleted file");
...
fn freshness_e2e_mixed_changes() {
    ...
    // 2. Add new file
    fs::write(repo_path.join("added.rs"), "pub const ADDED_CONTENT: usize = 123;\n").unwrap();
    ...
    let mod_result = index_search_with_overlay(&reader, &overlay, repo_path, "MODIFIED_MAIN").unwrap();
    assert!(!mod_result.results.matches.is_empty(), "should find modified content");

    // Test 2: Added content is searchable
    let add_result = index_search_with_overlay(&reader, &overlay, repo_path, "ADDED_CONTENT").unwrap();
    assert!(!add_result.results.matches.is_empty(), "should find added content");
    ...
    assert!(del_result.results.matches.is_empty(), "should not find content in deleted file");

    // Test 4: Unchanged file (src/utils.rs) is still searchable
    let unchanged_result = index_search_with_overlay(&reader, &overlay, repo_path, "helper").unwrap();
    assert!(!unchanged_result.results.matches.is_empty(), "should find content in unchanged file");
}
...
fn freshness_update_to_query_latency() {
    ...
    assert!(elapsed.as_secs() < 5, "update-to-query should be reasonably fast (< 5s for test)");
}
