...
impl GitRepo {
    ...
    pub fn head_commit(&self) -> Result<String, GitError> {
        ...
        let commit = head.peel_to_commit_in_place()
            .map_err(|e| GitError::InvalidReference(format!("HEAD is not a valid commit: {}", e)))?;
    ...
    pub fn detect_dirty_files(&self) -> Result<Vec<(PathBuf, FileStatus)>, GitError> {
        ...
        let workdir = self.repo.work_dir()
            .ok_or_else(|| GitError::OperationFailed("repository has no working directory".to_string()))?;
        ...
        for entry in index.entries() {
            ...
            if !abs_path.exists() {
                ...
                if let Ok(metadata) = abs_path.metadata() {
                    ...
                    let file_mtime_secs = metadata.modified()
                        .ok()
    ...
    pub fn resolve_reference(&self, refname: &str) -> Result<String, GitError> {
        let mut reference = self.repo.find_reference(refname)
            .map_err(|e| GitError::InvalidReference(format!("failed to find reference '{}': {}", refname, e)))?;
        
        let commit = reference.peel_to_id_in_place()
            .map_err(|e| GitError::InvalidReference(format!("reference '{}' is not a valid commit: {}", refname, e)))?;
...
mod tests {
    ...
    fn test_detect_modified_file() {
        ...
        let modified_files: Vec<_> = dirty.iter()
    ...
    fn test_detect_added_file() {
        ...
        let added_files: Vec<_> = dirty.iter()
    ...
    fn test_detect_deleted_file() {
        ...
        let deleted_files: Vec<_> = dirty.iter()
